...
void *Client(int *socket){
	...
	int ret;	
	int terminar=0;
	char username[50]= ""; //testing here, the username will take place in if 
	...
	while (terminar==0){
		...
		if (ret <= 0) {
			...
			if(strlen(username) > 0)
			{
				...
				if (res == -1)
					printf("The user wasn't logged in");
				else
					printf("The user %s was deleted from the list)", username);
				break;
			}
		}
		...
		char *p = strtok(input, "/");
		int codigo = atoi(p);
		char password[80], email[80]; //deleted the username here, since we need to access it out of the while loop
